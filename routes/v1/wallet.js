 ...
 */
router.get('/:account/stakeAddressRegistrationCertificate', function(req, res, next) {
    res.send(cardanoCli.stakeAddressRegistrationCertificate(req.params.account));
});

/**
 * Generate Stake Address Deresgistration Certificate and return file path
 */
router.get('/:account/stakeAddressDeregistrationCertificate', function(req, res, next) {
    res.send(cardanoCli.stakeAddressDeregistrationCertificate(req.params.account));
});

/**
 * Generate Stake Address Delegation Certificate and return file path
 */
router.get('/:account/stakeAddressDelegationCertificate', function(req, res, next) {
    res.send(cardanoCli.stakeAddressDelegationCertificate(
        req.params.account,
        req.params.poolId
    ));
});
